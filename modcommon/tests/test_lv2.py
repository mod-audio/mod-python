...
class BundleTest(unittest.TestCase):
    ...
    def test_plugins_are_properly_organized(self):
        ...
        self.assertEqual(len(bundle.data['plugins'].keys()), 18)

        m = bundle.data['plugins']['http://invadarecords.com/plugins/lv2/compressor/stereo']

        self.assertEqual(m['url'], 'http://invadarecords.com/plugins/lv2/compressor/stereo')
        self.assertEqual(m['name'], 'Invada Compressor (stereo)')
        self.assertEqual(m['maintainer']['name'], 'Invada')
        self.assertEqual(m['maintainer']['mbox'], 'fraser@arkhostings.com')
        self.assertEqual(m['maintainer']['homepage'],
                          'http://www.invadarecords.com/Downloads.php?ID=00000264')
        self.assertEqual(m['developer']['name'], 'Fraser Stuart')
        self.assertEqual(m['developer']['mbox'], 'fraser@arkhostings.com')
        self.assertEqual(m['developer']['homepage'],
                          'https://launchpad.net/invada-studio')
        self.assertEqual(m['license'], 'gpl')

        self.assertEqual(len(m['ports']['audio']['input']), 2)
        self.assertEqual(len(m['ports']['audio']['output']), 2)
        self.assertEqual(len(m['ports']['control']['input']), 8)
        self.assertEqual(len(m['ports']['control']['output']), 6)

        cs = m['ports']['control']['input']
        self.assertEqual(cs[0]['index'], 0)
        self.assertEqual(cs[0]['name'], 'Bypass')
        self.assertEqual(cs[0]['symbol'], 'bypass')
        self.assertEqual(cs[0]['default'], 0.0)
        self.assertEqual(cs[0]['minimum'], 0)
        self.assertEqual(cs[0]['maximum'], 1.0)
        self.assertEqual(cs[0]['logarithmic'], False)
        self.assertEqual(cs[1]['index'], 1)
        self.assertEqual(cs[1]['name'], 'RMS')
        self.assertEqual(cs[1]['symbol'], 'rms')
        self.assertEqual(cs[1]['default'], 0.5)
        self.assertEqual(cs[1]['minimum'], 0)
        self.assertEqual(cs[1]['maximum'], 1.0)
        self.assertEqual(cs[1]['logarithmic'], False)
        self.assertEqual(cs[3]['index'], 3)
        self.assertEqual(cs[3]['name'], 'Release')
        self.assertEqual(cs[3]['symbol'], 'release')
        self.assertEqual(cs[3]['default'], 0.05)
        self.assertEqual(cs[3]['minimum'], 0.001)
        self.assertEqual(cs[3]['maximum'], 5.0)
        self.assertEqual(cs[3]['logarithmic'], True)
        self.assertEqual(cs[3]['unit']['symbol'], 's')
        self.assertEqual(cs[6]['index'], 6)
        self.assertEqual(cs[6]['name'], 'Gain')
        self.assertEqual(cs[6]['symbol'], 'gain')
        self.assertEqual(cs[6]['default'], 0)
        self.assertEqual(cs[6]['minimum'], -6)
        self.assertEqual(cs[6]['maximum'], 36)
        self.assertEqual(cs[6]['unit']['symbol'], 'dB')
        self.assertEqual(cs[6]['logarithmic'], False)
    ...
    def test_scalepoints(self):
        ...
        self.assertEqual(len(port['scalePoints']), 36)
        self.assertEqual(port['scalePoints'][0], {'label': 'Sin', 'value': 0.0})
        self.assertEqual(port['scalePoints'][1], {'label': 'S0', 'value': 1.0})
        self.assertEqual(port['scalePoints'][35], {'label': 'P:Chant', 'value': 35.0})

    @attr(slow=1)
    def test_units(self):
        port = calf.data['plugins']['http://calf.sourceforge.net/plugins/Organ']['ports']['control']['input'][29]
        self.assertEqual(port['unit']['symbol'], 'ct')

    @attr(slow=1)
    def test_categories(self):
        inv = invada.data['plugins']
        self.assertEqual(inv['http://invadarecords.com/plugins/lv2/compressor/stereo']['category'],
                          ['Dynamics', 'Compressor'])
    ...
    def test_midi_ports(self):
        ...
        self.assertEqual(len(plugin['ports']['midi']['input']), 2)
        self.assertEqual(plugin['ports']['midi']['input'][0]['symbol'], 'event_in')
        self.assertEqual(plugin['ports']['midi']['input'][0]['name'], 'Event Midi')
        self.assertEqual(plugin['ports']['midi']['input'][1]['symbol'], 'atom_in')
    ...
    def test_bundle_id(self):
        ...
        try:
            ...
            self.assertEqual(Bundle(new_inv).data['_id'], inv_id)
    ...
    def test_plugin_id(self):
        ...
        try:
            ...
            self.assertEqual(new_comp['_id'], comp['_id'])
        finally:
            shutil.rmtree(new_inv)
    ...
    def test_plugin_checksum_is_compatible_with_mongo_objid(self):
        ...
        self.assertEqual(str(ObjectId(comp['_id'])), str(comp['_id']))
        self.assertEqual(str(ObjectId(inv['_id'])), str(inv['_id']))
    ...
    def test_plugin_package_data(self):
        ...
        self.assertEqual(comp['package_id'], inv['_id'])
        self.assertEqual(comp['package'], 'invada.lv2')

    @attr(slow=1)
    def test_plugin_url(self):
        comp = invada.data['plugins']['http://invadarecords.com/plugins/lv2/compressor/stereo']
        self.assertEqual(comp['url'], 'http://invadarecords.com/plugins/lv2/compressor/stereo')
    ...
    def test_units_path(self):
        ...
        try:
            ...
            self.assertEqual(invada.data['_id'], inv.data['_id'])
        finally:
    ...
    def test_version_and_stability(self):
        ...
        self.assertEqual(erreverb['version'], '0.0')
        self.assertEqual(erreverb['stability'], 'experimental')
        self.assertEqual(compressor['version'], '0.1')
        self.assertEqual(compressor['stability'], 'testing')
        self.assertEqual(delay['version'], '1.1')
        self.assertEqual(delay['stability'], 'unstable')
        self.assertEqual(filter['version'], '0.2')
        self.assertEqual(filter['stability'], 'stable')
...
class BundlePackageTest(unittest.TestCase):
    ...
    def test_packaging(self):
        ...
        try:
            ...
            self.assertEqual(bundle.data['_id'], invada.data['_id'])
        finally:
            os.chdir(cur_dir)
